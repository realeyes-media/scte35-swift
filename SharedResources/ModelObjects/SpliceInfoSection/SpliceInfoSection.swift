...
/// SpliceInfoSection is the Swifty version of SCTE-35 Standards as defined in the documents linked below. Use SCTE35Converter to parse a Base64String and create a SpliceInfoSection
...
public struct SpliceInfoSection {
    ...
    /// The section_syntax_indicator is a 1-bit field that should always be set to false, indicating that MPEG short sections are to be used.
    ...
    /// This is a 12-bit field specifying the number of remaining bytes in the splice_info_section, immediately following the section_length field up to the end of the splice_info_section. The value in this field shall not exceed 4093.
    ...
    /// When this bit is set to ‘1’, it indicates that portions of the splice_info_section, starting with splice_command_type and ending with and including E_CRC_32, are encrypted. When this bit is set to ‘0’, no part of this message is encrypted. The potentially encrypted portions of the splice_info_table are indicated by an E in the Encrypted column of Table 5.
    public let hasEncryptedPacket: Bool

    /// This 6-bit unsigned integer specifies which encryption algorithm was used to encrypt the current message. When the encrypted_packet bit is zero, this field is present but undefined. Refer to section 11, and specifically Table 27 - Encryption algorithm, for details on the use of this field.
    ...
    /// An 8-bit unsigned integer that conveys which control word (key) is to be used to decrypt the message. The splicing device may store up to 256 keys previously provided for this purpose. When the encrypted_packet bit is zero, this field is present but undefined.
    public let cwIndex: Int
    
    /// A 12-bit value used by the SCTE 35 message provider to assign messages to authorization tiers. This field may take any value between 0x000 and 0xFFF. The value of 0xFFF provides backwards compatibility and shall be ignored by downstream equipment. When using tier, the message provider should keep the entire message in a single transport stream packet.
    public let tier: Tier

    /// A 12-bit length of the splice command. The length shall represent the number of bytes following the splice_command_type up to, but not including the descriptor_loop_length. Devices that are compliant with this version of the standard shall populate this field with the actual length. The value of 0xFFF provides backwards compatibility and shall be ignored by downstream equipment.
    public let spliceCommandLength: Int

    /// An 8-bit unsigned integer which shall be assigned one of the values shown in column labeled splice_command_type value in Table 6.
    ...
    /// This is a 32-bit field that contains the CRC value that gives a zero output of the registers in the decoder defined in [MPEG Systems]after processing the entire splice_info_section, which includes the table_id field through the CRC_32 field. The processing of CRC_32 shall occur prior to decryption of the encrypted fields and shall utilize the encrypted fields in their encrypted state.
    public let crc32: String

    /// This is a 32-bit field that contains the CRC value that gives a zero output of the registers in the decoder defined in [MPEG Systems]after processing the entire decrypted portion of the splice_info_section. This field is intended to give an indication that the decryption was performed successfully. Hence, the zero output is obtained following decryption and by processing the fields splice_command_type through E_CRC_32.
